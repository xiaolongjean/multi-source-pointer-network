...
class BeamSearch:
    ...
    def search(self,
        ...
        merged_source_local_mask = (batch_input["merged_source_local_mask"].float() + 1e-45).log()
        ...
        for timestep in range(self.max_steps - 1):
            ...
            #print("time:", timestep,"\tshape: ",last_predictions.shape)
            ...
            # shape: (batch_size * beam_size, num_classes)
            #print(last_predictions.shape,"\n\n\n", broadcasted_global_ids.shape)

            last_predicted_local_ids = state["merged_source_local_ids"].gather(-1, last_predictions.unsqueeze(1)).squeeze(-1)
            last_predicted_global_ids = state["merged_source_global_ids"].gather(-1, last_predictions.unsqueeze(1)).squeeze(-1)
           # print(last_predicted_token_ids,"\n\n\n")
