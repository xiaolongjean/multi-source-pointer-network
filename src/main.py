...
def train(args):
    ...
    model =MS_Pointer(args, word2index = data_loader.word2index, 
                            char2index = data_loader.char2index, 
    ...
    for epoch in range(args.max_epoch):
        ...
        for idx, batch in enumerate(tqdm_generator):
            ...
            info = color("[Train] ",1) + "Epoch:" + color(epoch,2) + " Batch:" + color(idx,2) + " Loss:" + color(round(loss.detach().cpu().item(), 5),1) + " Time:" + color(batch_elapsed_time,2)
            tqdm_generator.set_description(desc=info, refresh=True)
            
        valid_start_time = time.time()
        total_valid_loss, mean_blue_score, valid_tokens, valid_probs = model.validation(valid_data_batchs, need_pred_result=True)
...
def test(args):
    ...
    data = [
            [["雪润", "皙白", "护肤套装", "化妆品", "礼盒品", "紧致", "提拉", "补水", "保湿", "粉白", "组合", "正品", "套装", "洁面膏", "冰肌水", "乳液", "精华"], 
             ["弹力", "补水", "国产", "保湿补水"]], 
            [["面霜", "正品", "专卖", "弹力", "蛋白", "凝时", "紧致霜", "补水", "保湿", "提拉", "紧致", "旗舰", "正品", "弹力", "蛋白", "凝时", "紧致霜"], 
             ["保湿", "紧致", "国产", "保湿补水"]], 
            [["京选", "抖音", "网红", "蜗牛", "保湿", "莹润", "嫩滑", "美肌", "补水", "保湿", "收缩", "毛孔", "面部", "护理", "套装"], 
             ["保湿", "补水", "国产", "干性", "保湿补水"]], 
            [["活泉", "水动力", "补水", "保湿", "护肤品", "套装", "洗面奶", "水乳液", "化妆品", "套装", "男女", "学生", "补水套装", "2件套", "水乳套装", "保湿水", "保湿乳"], 
             ["保湿", "补水", "国产", "干性", "补水套装2件套"]], 
            [["乳液面霜", "白肌", "精华乳液"], 
             ["保湿", "补水", "国产", "干性", "中性", "混合性", "保湿补水"]]
           ]
    
    pred_tokens, pred_probs = model.predict([item[0:2] for item in data2])
